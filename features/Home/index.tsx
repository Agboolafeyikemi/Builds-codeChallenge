  ...
  return (
    <div>
      <div className="flex justify-center">
        <button
          className={`${
            view === "email" ? "font-medium border" : undefined
          } border-gray-300 rounded-lg py-1.5 px-3 mr-4`}
          onClick={() => {
            setInputValue("");
            setView("email");
          }}
        ...
        <button
          className={`${
            view === "phone" ? "font-medium border" : undefined
          } border-gray-300 rounded-lg py-1.5 px-3`}
          onClick={() => {
            setInputValue("");
            setView("phone");
          }}
      ...
      >
        <div className="relative mb-5">
          {view === "email" ? (
            <input
              ...
              className="py-3 px-5 w-full h-full rounded-lg border border-gray-300 focus:border-purple-600 focus:outline-none"
            />
          ) : (
            <ReactPhoneInput
              country={"us"}
        ...
        <div className="flex justify-center w-full">
          <button
            disabled={
              (view === "email" && !validateEmail(inputValue)) ||
              (view === "phone" && inputValue.length < 4)
            }
            className={`font-semibold py-2.5 px-5 rounded-lg text-white flex items-center ${
              (view === "email" && validateEmail(inputValue)) ||
              (view === "phone" && inputValue.length > 4)
                ? "bg-purple-500"
                : "bg-gray-300"
